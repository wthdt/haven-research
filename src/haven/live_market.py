...
def summarize_breadth_rows(
    ...
    *,
    ...
    returns = (
        valid["return_1d"].dropna()
        if "return_1d" in valid
...
def build_live_breadth_snapshot(
    ...
    *,
    ...
    members, as_of = fetch_nasdaq100_members(
        cache_dir,
        force=force,
    ...
    timeout = (
        ...
        else float(batch_timeout_seconds)
    )
    ...
    futures: dict[Future, str] = {
        pool.submit(load, symbol): symbol for symbol in symbols
    }
    ...
    try:
        ...
        for future, symbol in futures.items():
            ...
            if future.done():
                ...
                continue
            future.cancel()
            timed_out_symbols.add(symbol)
            errors[symbol] = (
                "TimeoutError: live breadth batch exceeded "
                f"{timeout:g} seconds"
            )
    finally:
    ...
    summary["batch_timed_out"] = bool(timed_out_symbols)
    summary["timed_out_member_count"] = int(len(timed_out_symbols))
    summary["batch_timeout_seconds"] = timeout
