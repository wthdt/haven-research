...
def _cached_bytes(url: str, cache_path: Path, force: bool = False) -> bytes:
...
def fetch_nasdaq_history(
    ...
    payload = json.loads(_cached_bytes(url, path, force).decode("utf-8"))
...
def fetch_fred_series(
    ...
    payload = _cached_bytes(url, path, force)
...
def build_research_dataset(
    ...
    for symbol in symbols:
        history = fetch_nasdaq_history(symbol, start, end, raw_dir, force)
        dividends = fetch_nasdaq_dividends(symbol, raw_dir, force)
        histories[symbol] = add_total_return(history, dividends)

    fred_config = config["data"]["fred"]
    ndx = fetch_fred_series(fred_config["ndx"], raw_dir, force)
    high_yield = fetch_fred_series(
        fred_config["high_yield_spread"], raw_dir, force
    )
    cash_yield = fetch_fred_series(
        fred_config["cash_yield"], raw_dir, force
    )

    cboe_config = config["data"]["cboe"]
    vxn = fetch_cboe_history(cboe_config["vxn"], raw_dir, force)["CLOSE"]
    vix = fetch_cboe_history(cboe_config["vix"], raw_dir, force)["CLOSE"]
    vix3m = fetch_cboe_history(cboe_config["vix3m"], raw_dir, force)["CLOSE"]
